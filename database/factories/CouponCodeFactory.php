<?php

use Faker\Generator as Faker;

$factory->define(App\Models\CouponCode::class, function (Faker $faker) {
    // find a random type
    $type = $faker->randomElement(array_keys(App\Models\CouponCode::$typeMap));
    //create discount according type
    $value = $type === App\Models\CouponCode::TYPE_FIXED ? random_int(1,200) : random_int(1,50);

    //if fixed money, min amount should more than discount value
    if ($type === App\Models\CouponCode::TYPE_FIXED) {
        $minAmount = $value + 0.01;
    } else {
        //if % discount, 50% don't need min amount
        if (random_int(0,100) < 50) {
            $minAmount = 0;
        } else {
            $minAmount = random_int(100, 1000);
        }
    }

    return [
        'name'       => join(' ', $faker->words), // random name
        'code'       => App\Models\CouponCode::findAvailableCode(), // call coupon generate function
        'type'       => $type,
        'value'      => $value,
        'total'      => 1000,
        'used'       => 0,
        'min_amount' => $minAmount,
        'not_before' => null,
        'not_after'  => null,
        'enabled'    => true,
    ];
});
